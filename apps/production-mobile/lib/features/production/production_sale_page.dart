...
double result = 0;
    ...
    for (final product in _products) {
      ...
      final quantity = _quantityByProduct[rowKey] ?? 0;
...
String _productId(Map<String, dynamic> product) {
    return (product['product_id'] ?? product['id'] ?? '').toString().trim();
  }

  String _productRowKey(Map<String, dynamic> product) {
    final settingId =
        (product['inventory_product_setting_id'] ?? '').toString().trim();

    if (settingId.isNotEmpty) {
      return 'setting:$settingId';
    }

    final inventoryDocumentId =
        (product['inventory_document_id'] ?? '').toString().trim();

    if (inventoryDocumentId.isNotEmpty) {
      return 'inventory:$inventoryDocumentId';
    }
    ...
    return 'product:$productId|code:$inventoryCode|ice:$iceType';
  }
...
Map<String, dynamic> product,
    ...
    return _quantityControllers.putIfAbsent(
      rowKey,
      () => TextEditingController(
        text: (_quantityByProduct[rowKey] ?? 0) > 0
            ? (_quantityByProduct[rowKey] ?? 0).toString()
            : '',
      ),
    );
...
String value,
    ...
    setState(() {
      ...
      if (nextQuantity <= 0) {
        ...
        _quantityByProduct.remove(rowKey);

      } else {

        _quantityByProduct[rowKey] = nextQuantity;
...
List<Map<String, dynamic>> _buildItems() {
    ...
    for (final entry in _quantityByProduct.entries) {
      ...
      final product = _products.firstWhere(
        (row) => _productRowKey(row) == entry.key,
