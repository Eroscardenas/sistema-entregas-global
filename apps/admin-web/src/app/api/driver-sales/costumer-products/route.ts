...
function productMatchesInventorySetting(
  product: any,
  setting: any,
) {
  const pName = normalizeText(
    product?.nombre,
  );

  const pKind = normalizeText(
    product?.kind,
  );

  const pType =
    resolveProductIceType(
      product,
    );
...
async function loadInventoryOutputs(
  ...
  if (
    ...
    try {
      ...
      console.error(
        '[costumer-products] global-outputs devolvió una respuesta no JSON:',
        {
          status:
            res.status,
      ...
      throw new Error(
        `No se pudo consultar el inventario del chofer. ` +
          `global-outputs respondió HTTP ${res.status} con una respuesta inválida.`,
      );
    }
  }
   ...
   * IMPORTANTE:
   *
   * Antes un error de global-outputs se convertía en:
   *
   * new Map()
   *
   * Eso hacía que todo el stock apareciera en cero
   * aunque el verdadero problema fuera Firebase,
   * cuota agotada, error 500, etc.
   *
   * Ahora propagamos el error real.
   */
  if (!res.ok) {
    const message =
      String(
    ...
    console.error(
      '[costumer-products] global-outputs error:',
      {
        status:
          res.status,
    ...
    throw new Error(
      message ||
        `No se pudieron consultar las salidas de inventario (HTTP ${res.status}).`,
    );
  }
  ...
  if (
    ...
    throw new Error(
      String(
        json?.error ??
          'No se pudo obtener el inventario cargado del chofer.',
      ),
    );
  }
  ...
  if (
    !json.qtyByKey ||
    typeof json.qtyByKey !==
      'object'
  ) {
    throw new Error(
      'global-outputs no devolvió un inventario válido para el chofer.',
    );
  }
...
export async function GET(
  ...
  try {
    ...
    /*
     * =========================================================
     * MAPEO CHOFER -> INVENTARIO FIREBASE
     * =========================================================
     */
    let driverCode:
      string | null =
     ...
     * =========================================================
     *
     * Si Firebase/global-outputs falla,
     * esta llamada lanza error.
     *
    ...
    /*
     * =========================================================
     * PRODUCTOS CONFIGURADOS DEL CLIENTE
     * =========================================================
     */
    const {
      data:
    ...
    return NextResponse.json(
      ...
      {
        /*
         * Mantenemos JSON siempre.
         * 503 comunica mejor que el servicio de
         * inventario no está disponible temporalmente.
         */
        status: 503,
      },
    );
