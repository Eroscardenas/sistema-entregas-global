...
function buildKey(
  ...
  return `${normalizeCode(bolsaVaciaCodigo)}__${normalizeText(tipoHielo)}__${safeNum(pesoKg)}`;
}
...
export function useCommercialInventoryProduct() {
  ...
  const [settingsMap, setSettingsMap] = useState<Map<string, InventoryProductSettingRow>>(new Map());
    ...
    async function loadSettings() {
      try {
        ...
        const { data, error } = await sb
          ...
          .select(
            'id, firebase_bolsa_vacia_codigo, firebase_tipo_hielo, peso_kg, nombre_comercial, precio_base, activo'
          );

        if (error) throw error;

        const next = new Map<string, InventoryProductSettingRow>();

        for (const row of (data ?? []) as InventoryProductSettingRow[]) {
          const key = buildKey(
        ...
        if (!cancelled) {
          console.error('Error cargando inventory_product_settings:', e);
          setSettingsMap(new Map());
          setError(e?.message ?? 'No se pudo cargar la configuración comercial');
        }
      } finally {
        if (!cancelled) {
  ...
  const products = useMemo<CommercialInventoryProduct[]>(() => {
    return (inventoryProducts ?? [])
      .map((row: InventoryProductRealtime) => {
        ...
        return {
          ...
          nombreComercial: setting?.nombre_comercial ?? null,
          precioBase:
            setting?.precio_base != null
              ? safeNum(setting.precio_base, 0)
              : null,
          activoComercial: Boolean(setting?.activo ?? false),
          configured: Boolean(setting?.id),
        };
      })
      .sort((a, b) => {
  ...
  return {
    products,
    loading: inventoryLoading || loadingSettings,
    error,
  };
}